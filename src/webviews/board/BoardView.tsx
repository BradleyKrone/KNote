...
// source notes through the host RPC.
...
export function BoardView({ scope }: { scope: BoardScope }): React.JSX.Element {
  ...
  // While a column-change commit is in flight (host write + reindex round
  // trip), keep showing the drag preview in place instead of snapping back.
  ...
  const onDragEnd = (event: DragEndEvent): void => {
    ...
    if (!sameColumn) {
      ...
      if (card.path !== overCard.path) {
        showToast('Cards keep note order — reorder works within one note')
      }
      clearPreview()
    }
  }
