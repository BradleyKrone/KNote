...
import { defaultReasonDate, useReasonPromptStore } from '@/stores/reasonPromptStore'
...
export function ReasonDialog(): React.JSX.Element | null {
  ...
  useEffect(() => {
    ...
    // Focus after the dialog mounts so autoFocus survives the state reset above.
    ...
    <div className="modal-overlay" onMouseDown={() => answer(null)}>
      <div className="modal-panel confirm-panel" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <label className="reason-date-field">
          <span className="settings-label">Since</span>
          <input
            type="date"
